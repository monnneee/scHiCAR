...
### Output files:
- The `02_fragment/*.tsv.gz` files can be used in standard scATAC-seq downstream analysis (such as with [ArchR](https://www.archrproject.com/articles/Articles/tutorial.html)), starting with **reformatFragmentFiles** to prepare for **createArrowFiles**).
- The `02_fragment/*.cut_rank` files list high-quality barcodes, which can be used to filter ATAC fragments either before or after the `createArrowFiles` step in ArchR.
