...
for root, dirs, files in os.walk(args.fastq_dir):
    for file in files:
        if file.endswith("fastq.gz"):
            ...
            #m = re.search(r"(.*)_L[0-9]{2}_(R[12]).fq.gz",file) ## get the sample lane
            m = re.search(r"(.*)_(R[12])_[0-9]{3}.fastq.gz",file) ## get the sample lane
            #m = re.search(r"(.*)_([12]).fastq.gz",file) ## get the sample lane
            #m = re.search(r"(.*)_L[0-9]{3}_(R[12])_[0-9]{3}.fastq.gz",file) ## get the sample lane
            if m:
                sample = m.group(1) 
        ...
        if file.endswith("fq.gz"):
            ...
            #R1 will be forward reads, R2 will be reverse reads
            #m = re.search(r"(.*)_L[0-9]{2}_(R[12]).fq.gz",file) ## get the sample lane
            #m = re.search(r"(.*)_(read_[12]).fq.gz",file) ## get the sample lane
