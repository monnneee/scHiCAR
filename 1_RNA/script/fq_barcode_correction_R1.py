...
def update_fastq(r1,r2,out_r1,out_r2, barcode_dic,white_list_barcode,log_file): ## process one files
    ...
    while True:
        ...
        discr = cur_r1_name.split()
        cur_barcode = (discr[-2][18:24].upper() + discr[-1][9:15].upper() + discr[-2][24:30].upper())  ## makesure the barcode length
        cur_umi = (discr[-2][12:18].upper() + discr[-2][30:36].upper() + cur_r2_read[-5:] + cur_r2_read[21:26])  ## makesure the barcode length
        ...
        if  cur_barcode in barcode_dic or cur_barcode in white_list_barcode : ## only the whitelist or corrected barcode are remained
            ...
            cur_r1_name = (cur_barcode + cur_umi + cur_r1_name) ## 18bp cell barcode
            cur_r1_read = (cur_barcode + cur_umi) ## 18bp cell barcode
            cur_r1_qual = "FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF"
            cur_r2_name = (cur_barcode + cur_umi + cur_r2_name) 
...
white_list_barcode = from_file_to_barcode_list('linear_sciRNA_18bp_barcode.txt.gz')
